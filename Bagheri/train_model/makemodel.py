...
node2vec_model = Word2Vec.load("../model/" + mode + "/node2vec_model-" + str(s) + ".model")
node_vectors = node2vec_model.wv
print(node_vectors.key_to_index)
node_id = "if"
print(f"Embedding for node {node_id}: {node_vectors[str(node_id)]}")
...
# #bilstm
# combined_features = np.concatenate((word_vectors_common_array, node_vectors_common_array), axis=-1)
# combined_features = combined_features.reshape((combined_features.shape[0], combined_features.shape[1], 1))  # 形状为 (n, 600, 1)
# model = Sequential()
# model.add(Bidirectional(LSTM(150, return_sequences=False), input_shape=(combined_features.shape[1], combined_features.shape[2])))  # 输出为 (n, 300)
                ...
                if "source" in data[r][c]["files"][f]:
                    ...
                    for change in data[r][c]["files"][f]["changes"]:
                        ...
                        for bad in badparts:
                            ...
                            if not -1 in pos:  # 表示在源代码中找到了bad代码
                                allbadparts.append(bad)
                    if (len(allbadparts) > 0):
                        positions = myutils.findpositions(allbadparts, sourcecode)
                        blocks = myutils.getblocks(sourcecode, positions, step, fulllength, mode)
                        for b in blocks:
                            allblocks.append(b)
...
numpy.save('key_data/keystrain_data_' + mode + '.npy', keystrain_data)
numpy.save('key_data/keystest_data_' + mode + '.npy', keystest_data)
numpy.save('key_data/keysfinaltest_data_' + mode + '.npy', keysfinaltest_data)
...
X_finaltest = sequence.pad_sequences(X_finaltest, maxlen=100)
...
# 创建模型
model = Sequential()
model.add(Bidirectional(LSTM(neurons, return_sequences=True), input_shape=(100, 600)))  # around 50 seems good
...
for dataset in ["train", "test", "finaltest"]:
    ...
    if dataset == "train":
        ...
        tn, fp, fn, tp = confusion_matrix(y_train, yhat_classes).ravel()
        FPR = fp / (fp + tn)
        FNR = fn / (fn + tp)
    ...
    if dataset == "finaltest":
        ...
        tn, fp, fn, tp = confusion_matrix(y_finaltest, yhat_classes).ravel()
        FPR = fp / (fp + tn)
        FNR = fn / (fn + tp)
